...
//            "SELECT creditstable.id, creditstable.money, creditstable.month, creditstable.name, creditstable.creditsDecimal, creditstable.payment FROM creditstable JOIN credit_user ON creditstable.id = credit_user.credit_id WHERE credit_user.user_id = ?;";
            "SELECT creditstable.id, creditstable.money, creditstable.month, creditstable.name, creditstable.creditsDecimal, credit_user.money FROM creditstable JOIN credit_user ON creditstable.id = credit_user.credit_id WHERE credit_user.user_id = ?;";
    ...
    public List<Credit> getAllByUserId(String command) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                credit.setName(rs.getString(4));
                credit.setDecimal(rs.getInt(5));
                credit.setSumma(rs.getInt(2)+(rs.getInt(2)*rs.getInt(5)/100));
                credit.setPayment(rs.getInt(6));
